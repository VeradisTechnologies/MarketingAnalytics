        ...
        try:
            ...
            if period and period_year:
                if period == "day" or period == "week" and period_month:
                    cursor.callproc("usp_report_ga_visitor", [str(period),str(period_year),str(period_month),chart_name,visitor_type,company_id])
                else:    
                    cursor.callproc("usp_report_ga_visitor", [str(period),str(period_year), None,chart_name,visitor_type,company_id])
            else:
                cursor.callproc("usp_report_ga_visitor", ['month',str(datetime.now().year), None,chart_name,visitor_type,company_id])
...
def gender_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            ...
            cursor.callproc("usp_report_ga_gender", [period, period_year, period_month,company_id])
...
def agegroup_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            ...
            cursor.callproc("usp_report_ga_AgeGroup", [period, period_year, period_month,company_id])
...
def conversion_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            ...
            cursor.callproc("usp_report_ga_conversion_rate_day", [period, period_year, period_month,chart_name,visitor_type,company_id])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():
...
def interacton_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            ...
            cursor.callproc("usp_report_ga_Interaction_per_visit", [period, period_year, period_month, chart_name,visitor_type,company_id ])
            if cursor.rowcount > 0:
                for i in cursor.fetchall():
...
def device_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            ...
            cursor.callproc("usp_report_ga_Device", [period, period_year, period_month,chart_name,visitor_type,company_id])
            if cursor.rowcount > 0:            
                for i in cursor.fetchall():
...
def city_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            ...
            cursor.callproc("usp_report_ga_City", [period, period_year, period_month,chart_name,visitor_type,company_id])
            if cursor.rowcount > 0:
                for i in cursor.fetchall():
                    if i:
...
def total_page_data(request):
    ...
    if is_valid_session(request):
        ...
        page_name = []
        page_count = []
        context ={'data':False, 'page_name':[], 'page_count':[]}
        ...
        try:
            ...
            cursor.callproc("usp_report_ga_Pages", [period, period_year, period_month,company_id])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():
...
def pageview_data(request):
    ...
    if is_valid_session(request):
        ...
        cursor = connection.cursor()
        try:
            cursor.callproc("usp_report_ga_Pageviews", [period, period_year, period_month, s_date, e_date, q_drilldown, company_id])
...
def ga_summary_top_chart(request):    
    """
        This method is used to fetch  summary top chart data for google analytics
    """
    period = request.GET.get('type')
    period_year = request.GET.get('year')
    period_month = request.GET.get('month')
    ...
    if is_valid_session(request):
        context = {'data':False,'visits':[],'new_users':[],'avg_page_views':[],'avg_duration':[],'bounce_rate':[]}
        ...
        try:  
            ...
            cursor.callproc("usp_report_ga_summary", [period, period_year, period_month])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():
                    context['visits']=int(i[0])
                    context['new_users']=int(i[1])
                    context['avg_page_views']=int(i[2])
                    context['avg_duration']=int(i[3])
                    context['bounce_rate']=int(i[4])
...
def engagement_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        instagram_data = []
        twitter_data=[]
        context = {'data':False, 'x_axis_data':[], 'facebook_data':[], 'instagram_data':[],"twitter_data":[]}
        period_index = {"Daily": "day", "Weekly":"week", "Monthly":"month", "Quarterly":"quarter"}
        cursor = connection.cursor()
        
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_engagements", [period, period_year, period_month, '', '',company_id])
            ...
            if cursor.rowcount > 0:
                for i in cursor.fetchall():
                    ...
                    twitter_data.append(int(i[3]))
                # print(twitter_data,"twitter engagenments")
...
def impressions_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_impressions_v1", [period, period_year, period_month, company_id])
            if cursor.rowcount > 0:
                for i in cursor.fetchall():
...
def fans_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_followers_total", [period, period_year, period_month,company_id])
            if cursor.rowcount > 0:
...
def clicks_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_Clicks", [period, period_year, period_month,company_id])
...
def likes_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_likes_v1", [period, period_year, period_month,company_id])    
            if cursor.rowcount > 0:
                for i in cursor.fetchall():
                    x_axis_data.append(i[0]) # i[0] - period type
...
def video_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_VideoViews", [period, period_year, period_month,company_id])
            if cursor.rowcount > 0:
                for i in cursor.fetchall():
                    x_axis_data.append(i[0]) # i[0] - period type
...
def fb_agegroup_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_Facebook_AgeGroup", [period, period_year, period_month,company_id])
            if cursor.rowcount > 0:
                for i in cursor.fetchall():
...
def fb_tile_bar_data(period,period_year,period_month,company_id):
    """
        This method is used to fetch facebook tile bar data
    """      
    context = {'fb_data': False, 'Daily impressions': 0, 'Paid Likes': 0, 'Organic Likes': 0, 
    ...
    try:
        ...
        cursor.callproc("usp_report_Facebookdata", [period, period_year, period_month,company_id])
        if cursor.rowcount > 0:
            context['fb_data'] = True
            for i in cursor.fetchall():
                context[i[0]] = int(i[1]) # i[0] - label ,i[1] - count
                # print(i[0]]," fb tile bar data")
                # print(int(i[1])," fb tile99999 bar data")  
        else:
            context = {'fb_data':False}                                
    except Exception as e:  
        context = {'fb_data':False}          
        logger.error("#"*10+'\t'+str(e))
    finally:
...
def fb_total_likes_fans(period,period_year,period_month):
    """
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            for i in cursor.fetchall():
                context[i[0]] = int(i[1]) # i[0] - label ,i[1] - count
                # print(context[i[0]],"totla fans likes")
        else:
            context = {'data':False}
    except Exception as e:
        context = {'data':False}
...
def tw_tile_bar_data(period,period_year,period_month,company_id):
    """
        This method is used to fetch twitter tile bar data
    """    
       
    context = {'tw_data': False, 'Daily Impressions': 0, 'Daily retweet count': 0, 'Daily relpy count': 0, 
    ...
    try:
        ...
        cursor.callproc("usp_report_twitterdata", ['all', period_year, period_month,company_id])
        if cursor.rowcount > 0:
            context['tw_data'] = True
            for i in cursor.fetchall():
...
def insta_tile_bar_data(period,period_year,period_month,company_id):
    """
        This method is used to fetch instagram tile bar data
    """            
    context = {'insta_data': False, 'Impressions': 0, 'Engagements': 0, 'Likes': 0, 'Followers': 0, 'Comments': 0}
    cursor = connection.cursor()
    ...
    try:
        ...
        cursor.callproc("usp_report_Instagramdata", [period, period_year, period_month,company_id])
        if cursor.rowcount > 0:
            context['insta_data'] = True
            for i in cursor.fetchall():
                context[i[0]] = int(i[1]) # i[0] - label ,i[1] - count
        else:
            context = {'insta_data':False}
    except Exception as e:
        context = {'insta_data':False}
        logger.error("#"*10+'\t'+str(e))
    finally:
...
def insta_total_likes_followers(period,period_year,period_month):
    """
...
def fb_most_views_post_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_top_fb_post", [period, period_year, period_month, selected_period,company_id])
...
def gender_data_social(request):
    ...
    if is_valid_session(request):
        ...
        try:
            ...
            cursor.callproc("usp_report_Gender", [period, period_year, period_month,company_id])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():
...
def fb_total_summary(request):
    ...
    if is_valid_session(request):
        company_id=request.session.get('company_id')  	
        try:
           fb_tile_bar_context=fb_tile_bar_data(period,period_year,period_month,company_id) 
        except Exception as e:
            fb_tile_bar_context = {'fb_data':False}
            logger.error("#"*10+'\t'+str(e))        
        
        # try:  
        #     fb_total_likes_fans_context=fb_total_likes_fans(period,period_year,period_month,company_id)                      
        # except Exception as e:
        #     fb_total_likes_fans_context = {'data':False}
        #     logger.error("#"*10+'\t'+str(e))

        try:
            insta_tile_bar_context=insta_tile_bar_data(period,period_year,period_month,company_id)
        except Exception as e:
            insta_tile_bar_context = {'insta_data':False}
            logger.error("#"*10+'\t'+str(e))
        
        # try:
        #     insta_total_likes_followers_context=insta_total_likes_followers(period,period_year,period_month,company_id)
        # except Exception as e:
        #     insta_total_likes_followers_context = {'data':False}
        #     logger.error("#"*10+'\t'+str(e))

        try:
            tw_tile_bar_data_context=tw_tile_bar_data(period,period_year,period_month,company_id)
        except Exception as e:
            tw_tile_bar_data_context = {'tw_data':False}
            logger.error("#"*10+'\t'+str(e))
        


        context={**fb_tile_bar_context,**insta_tile_bar_context,**tw_tile_bar_data_context}
        return JsonResponse(context)
    else:
        return JsonResponse({'data':False,'status':403})
...
def opens_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_mc_total_unique_open", [period, period_year, period_month, company_id])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():
                    x_axis_data.append(i[0])  # i[0] - period type
...
def click_through_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_mc_click_through", [period, period_year, period_month, selected_period, company_id])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():
                    x_axis_data.append(i[0]) # i[0] - period type
...
def email_send_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period = period_index.get(period)
            cursor.callproc("usp_report_mc_email_send", [period, period_year, period_month, selected_period, company_id])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():
...
def top_campaign_chart_data(request):
    ...
    if is_valid_session(request):
        ...
        try:
            period=period_index.get(period)
            cursor.callproc("usp_report_mc_Top_campaign", [period, period_year, period_month, selected_period, company_id])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():
    ...
    if is_valid_session(request):
        ...
        try:  
            ...
            if period == "week":
                period="day"
            # cursor.callproc("usp_report_mc_mail_campaigns", [period, period_year, period_month, selected_period])
            cursor.callproc("usp_report_mc_mail_campaigns", [period, period_year, period_month, selected_period, company_id])
            if cursor.rowcount > 0:
                context['data'] = True
                for i in cursor.fetchall():                
