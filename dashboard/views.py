...
from .models import CustomerDetails, SocialMedia, GoogleAnalytics, Mailchimp, UserCompanyMapping
    ...
    if request.method == 'POST':
        ...
        if login_form.is_valid():
            ...
            if authenticate(username=username, password=password) and user:
                company_obj=UserCompanyMapping.objects.filter(user_id=user.id).first()
                ...
                request.session['company_id'] = company_obj.company.companyid
