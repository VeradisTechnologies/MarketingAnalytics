...
function ajax_agegroup_chart(period, year_val = null, month_val = null) {
    ...
    $("#age_gender_charts_ga_nodata").hide()
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data["data"] == true) {
                ...
                $("#age_gender_charts_ga_nodata").show()
            }
        }
    });
}
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data["data"] == true) {
                ...
                $("#top5_chart_nodata").show()
            }
        }
    });
}

function ajax_ga_summary_top_chart(period, selected_period=''){
    selected_month = $("#month_dropdown option:selected").val()
    selected_year = $("#year_dropdown option:selected").val()

    $.ajax({
        url: "endpoint/ga_summary_top_chart",
        data: { "type": '' + period, 'year': selected_year, 'month': selected_month, 'selected_period':selected_period },
        success: function (data) {            
            if (data['data'] == true) {
                                
                $("#ga_visits").text(Number( data['total_campaign'] ).toLocaleString('en-us'))
                $("#ga_bounce_rate").text(Number( data['total_mail_sent'] ).toLocaleString('en-us'))
                $("#ga_avg_page_views").text(Number( data['click_throughs'] ).toLocaleString('en-us'))
                $("#ga_avg_duration").text(Number( data['total_opens'] ).toLocaleString('en-us'))
                $("#ga_new_users").text(Number( data['spam_complaints'] ).toLocaleString('en-us'))

            } else {
                $("#ga_visits").text('NA')
                $("#ga_bounce_rate").text('NA')
                $("#ga_avg_page_views").text('NA')
                $("#ga_avg_duration").text('NA')
                $("#ga_new_users").text('NA')
            }
        }
    });
}
